...
class SelectedSubscriptionYearly extends StatefulWidget {
  final String index;
  final String title;
  final String? desc1;
  final String? desc2;
  ...
  const SelectedSubscriptionYearly(
      {Key? key,
        required this.index,
        required this.title,
        this.desc1,
  ...
  @override
  State<SelectedSubscriptionYearly> createState() => _SelectedSubscriptionYearlyState();
}

class _SelectedSubscriptionYearlyState extends State<SelectedSubscriptionYearly> {
  ...
  Future<void> sendSubscriptionData() async {
    ...
    final response = await http.post(purchasedSubscription, body: {
      ...
      'expiry_date': DateTime.now().add(Duration(days: 365)).toString(),
    });
